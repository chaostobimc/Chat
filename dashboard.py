...
def panels_page():
    ...
    # NEW: Load channels and roles for the "Panel Senden" modal
    channels = db.execute("SELECT * FROM guild_channels ORDER BY channel_name").fetchall()
    roles = db.execute("SELECT * FROM guild_roles ORDER BY role_name").fetchall()
    guilds = db.execute("""
        SELECT DISTINCT guild_id FROM guild_channels 
        WHERE guild_id != '' AND guild_id IS NOT NULL
    """).fetchall()
    ...
    return render_template('panels.html', panels=panels,
                           ...
                           guilds=[row['guild_id'] for row in guilds])
...
def create_button():
    ...
    # NEW: Collect multiple support roles
    support_role_ids = request.form.get('support_role_ids', '')
    
    db = get_db()
    db.execute("""
        INSERT INTO ticket_buttons 
        (panel_id, custom_id, label, emoji, color, description, 
         category_id, support_role_id, support_role_ids, question_title, question_placeholder,
    ...
    if request.method == 'POST':
        ...
        db.execute("""
            UPDATE ticket_buttons SET 
                ...
                category_id = ?, support_role_id = ?, support_role_ids = ?,
                question_title = ?, question_placeholder = ?,
                require_question = ?, max_tickets = ?,
                image_url = ?, image_local = ?,
                use_modal = ?, modal_title = ?, modal_fields = ?
            WHERE id = ?
        """, (label, emoji, color, description, category_id, support_role_id, support_role_ids,
...
# ==================== NEW: PANEL SEND & GUILD DATA ====================

@app.route('/panels/<int:panel_id>/send', methods=['POST'])
@login_required
def send_panel_to_discord(panel_id):
    """Queue a panel to be sent to a Discord channel (processed by bot)."""
    channel_id = request.form.get('channel_id')
    guild_id = request.form.get('guild_id', '')
    
    if not channel_id:
        flash('Bitte wähle einen Kanal aus!', 'error')
        return redirect(url_for('panels_page'))
    
    # If no guild_id provided, try to find it from the panel or use first guild
    if not guild_id:
        db = get_db()
        panel = db.execute("SELECT * FROM ticket_panels WHERE id = ?", (panel_id,)).fetchone()
        if panel and panel['guild_id'] and panel['guild_id'] != 'dashboard':
            guild_id = panel['guild_id']
        else:
            # Try to determine guild from channels
            channels = db.execute("SELECT DISTINCT guild_id FROM guild_channels LIMIT 1").fetchone()
            if channels:
    ...
    flash('Panel wird in Kürze an den Discord-Kanal gesendet! (Der Bot verarbeitet die Anfrage automatisch)', 'success')
    return redirect(url_for('panels_page'))
...
@login_required
def api_channels():
    """API endpoint returning cached guild channels for the dashboard."""
...
@login_required
def api_roles():
    """API endpoint returning cached guild roles for the dashboard."""
    guild_id = request.args.get('guild_id', '')
    db = get_db()
    if guild_id:
        roles = db.execute(
...
@login_required
def api_guilds():
    """API endpoint returning cached guild IDs for the dashboard."""
    db = get_db()
    guilds = db.execute("""
        SELECT DISTINCT guild_id FROM guild_channels 
        WHERE guild_id != '' AND guild_id IS NOT NULL
    """).fetchall()
    db.close()
    return jsonify([row['guild_id'] for row in guilds])
